...
//3、根据城市名和年份查所有采购信息：http://localhost:8080/Tendering/caigou/getCaigouByYearAndCity?years=2015&city=韶关市
//4、根据年份查找城市采购总额：http://localhost:8080/tenderingcaigou/getCaigouListByYear?years=2017
//5、根据项目类型模糊查找采购金额：http://localhost:8080/tenderingcaigou/getCaigouAllByItemType?itemType=其他专业技术服务
//
...
//8、根据年份单位名查询同类型设计机构在每个市的中标总金额：http://localhost:8080/tenderingBidwin/getSumByCompanyYear?WinbidCompany=广东省城乡规划设计研究院&years=2018
//
//http://localhost:8080/tenderingBidwin/getSumByCompanyYear?WinbidCompany=广东省城乡规划设计研究院&years=2018
$(function () {
    var selectTable = $('#seleTable option:selected').val();
    if( selectTable === "1" ){
        $("#txt_search_company").attr("disabled", true);
    }
    if( selectTable === "2" ){
        $("#txt_search_company").attr("disabled", false);
    }
    $("#seleTable").change(function () {
        if($("select option").is(":selected")){
            selectTable = $('#seleTable option:selected').val();
            if( selectTable === "1" ){
                $("#txt_search_company").attr("disabled", true);
            }
            if( selectTable === "2" ){
